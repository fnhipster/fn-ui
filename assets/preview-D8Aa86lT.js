...
`;class m extends HTMLElement{constructor(){super();a(this,"timeElement",null);this.attachShadow({mode:"open"}).appendChild(C.content.cloneNode(!0)),this.timeElement=this.shadowRoot.querySelector("time")}static get observedAttributes(){return["date"]}attributeChangedCallback(t,e,s){if(t==="date"){const i=parseInt(s,10),r=new Date(i);this.timeElement.setAttribute("datetime",r.toISOString()),this.timeElement.textContent=m.timeAgo(s)}}static timeAgo(t){const s=Date.now()-t,i=Math.floor(s/1e3),r=Math.floor(i/60),h=Math.floor(r/60),l=Math.floor(h/24),c=Math.floor(l/30),d=Math.floor(l/365);return i<60?"just now":r<60?`${r} minute${r!==1?"s":""} ago`:h<24?`${h} hour${h!==1?"s":""} ago`:l<30?`${l} day${l!==1?"s":""} ago`:c<12?`${c} month${c!==1?"s":""} ago`:`${d} year${d!==1?"s":""} ago`}}customElements.define(et,m);const ot="fn-head",L=document.createElement("template");L.innerHTML=`
  <style>
    :host {
      display: flex;
