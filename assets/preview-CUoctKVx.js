var T=Object.defineProperty;var N=(n,o,t)=>o in n?T(n,o,{enumerable:!0,configurable:!0,writable:!0,value:t}):n[o]=t;var i=(n,o,t)=>N(n,typeof o!="symbol"?o+"":o,t);import{N as A}from"./index-B0EJvtKl.js";import"./_commonjsHelpers-Cpj98o6Y.js";const U="fn-binary";class P extends HTMLElement{constructor(){super();i(this,"playing");i(this,"current",0);i(this,"binary",[]);i(this,"intersectionObserver",new IntersectionObserver(t=>{t.forEach(e=>{e.isIntersecting?this.play():this.stop()})}));this.attachShadow({mode:"open"})}static get observedAttributes(){return["message"]}connectedCallback(){this.intersectionObserver.observe(this),this.addEventListener("mouseenter",this.stop),this.addEventListener("mouseleave",this.play)}disconnectedCallback(){this.stop(),this.intersectionObserver.unobserve(this),this.removeEventListener("mouseenter",this.stop),this.removeEventListener("mouseleave",this.play)}attributeChangedCallback(t,e,s){if(e!==s&&t==="message"){const r=this.getAttribute("message");this.binary=r.split("").map(a=>a.charCodeAt(0).toString(2).padStart(8,"0"))}}play(){if(this.playing)return;const t=this.binary.length-1;this.render(this.binary[this.current]),this.playing=setInterval(()=>{this.current===t?this.current=0:this.current+=1,this.render(this.binary[this.current])},2e3)}stop(){clearInterval(this.playing),this.playing=void 0}render(t){t&&(this.shadowRoot.innerHTML=t)}}customElements.define(U,P);const B="fn-footer",p=document.createElement("template");p.innerHTML=`
  <style>
    :host {
      align-items: center;
      display: flex;
...
`;class _ extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}).appendChild(u.content.cloneNode(!0))}}customElements.define(V,_);const D="fn-link",v=document.createElement("template");v.innerHTML=`
  <style>
    ::slotted(a) {
...
`;class q extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}).appendChild(f.content.cloneNode(!0))}}customElements.define(K,q);const j="fn-image",w=document.createElement("template");w.innerHTML=`
  <style>
    :host {
      --corner: 1rem;
...
`;class z extends HTMLElement{constructor(){super();i(this,"loaded",!1);i(this,"observer",new IntersectionObserver(t=>{t.forEach(e=>{e.isIntersecting&&(this.shadowRoot.querySelector(".wrapper").classList.add("loaded"),this.handleGlow(),this.loaded=!0,this.observer.disconnect())})}));this.attachShadow({mode:"open"}).appendChild(w.content.cloneNode(!0))}async connectedCallback(){var e,s;if(this.loaded)return;this.image=this.querySelector("img"),this._handlePoster=this.handlePoster.bind(this),this._handleLoad=()=>{this.observer.observe(this.image)};const t=this.getAttribute("poster");t&&(this.poster=new Image,this.poster.src=t,this.poster.complete?this._handlePoster():this.poster.addEventListener("load",this._handlePoster)),(e=this.image)!=null&&e.complete?this._handleLoad():(s=this.image)==null||s.addEventListener("load",this._handleLoad)}disconnectedCallback(){var t,e;(t=this.image)==null||t.removeEventListener("load",this._handleLoad),(e=this.poster)==null||e.removeEventListener("load",this._handlePoster),this.observer.disconnect()}handlePoster(){const t=this.shadowRoot.querySelector("canvas#poster"),e=t.getContext("2d"),{width:s,height:r}=this.image;t.width=s,t.height=r;const a=.05,h=t.width*a,l=t.height*a;e.drawImage(this.poster,0,0,h,l),e.msImageSmoothingEnabled=!1,e.mozImageSmoothingEnabled=!1,e.webkitImageSmoothingEnabled=!1,e.imageSmoothingEnabled=!1,e.drawImage(t,0,0,h,l,0,0,t.width,t.height);const d=getComputedStyle(this).getPropertyValue("--color-fg");e.globalCompositeOperation="color",e.fillStyle=d,e.fillRect(0,0,t.width,t.height),t.classList.add("loaded")}handleGlow(){const t=this.shadowRoot.querySelector("canvas#glow"),e=t.getContext("2d"),{width:s,height:r}=this.image;t.width=s,t.height=r,t.style.filter="blur(1.5rem) opacity(0.5)",e.drawImage(this.image,0,0,s,r),t.classList.add("loaded")}}customElements.define(j,z);const O="fn-columns",y=document.createElement("template");y.innerHTML=`
  <style>
    :host {
      --gap: var(--spacing-sm);
      display: grid;
  ...
  <slot name="row"></slot>
`;class Z extends HTMLElement{static get observedAttributes(){return["columns"]}constructor(){super(),this.attachShadow({mode:"open"}).appendChild(y.content.cloneNode(!0))}connectedCallback(){this.style.setProperty("--size",this.children[0].children.length),[...this.children].forEach(o=>{o.style.setProperty("display","grid"),o.style.setProperty("grid-gap","var(--gap)"),o.style.setProperty("grid-template-columns","repeat(var(--columns), 1fr)")})}}customElements.define(O,Z);const X="fn-hero",b=document.createElement("template");b.innerHTML=`
  <style>
    :host {
      display: block;
      position: relative;
...
`;class tt extends HTMLElement{static get observedAttributes(){return["title","text","url"]}constructor(){super(),this.attachShadow({mode:"open"}).appendChild(E.content.cloneNode(!0))}connectedCallback(){this.shadowRoot.querySelector(".fnh-share").addEventListener("click",this.share.bind(this))}share(){navigator.share?navigator.share({title:this.getAttribute("title"),text:this.getAttribute("text"),url:this.getAttribute("url")}):navigator.clipboard.writeText(this.getAttribute("url"))}}customElements.define(Q,tt);const et="fn-timeago",C=document.createElement("template");C.innerHTML=`
  <style>
    :host {
      font: var(--font-accent);
      text-transform: uppercase;
...
`;class g extends HTMLElement{constructor(){super();i(this,"timeElement",null);this.attachShadow({mode:"open"}).appendChild(C.content.cloneNode(!0)),this.timeElement=this.shadowRoot.querySelector("time")}static get observedAttributes(){return["date"]}attributeChangedCallback(t,e,s){if(t==="date"){const r=parseInt(s,10),a=new Date(r);this.timeElement.setAttribute("datetime",a.toISOString()),this.timeElement.textContent=g.timeAgo(s)}}static timeAgo(t){const s=Date.now()-t,r=Math.floor(s/1e3),a=Math.floor(r/60),h=Math.floor(a/60),l=Math.floor(h/24),c=Math.floor(l/30),d=Math.floor(l/365);return r<60?"just now":a<60?`${a} minute${a!==1?"s":""} ago`:h<24?`${h} hour${h!==1?"s":""} ago`:l<30?`${l} day${l!==1?"s":""} ago`:c<12?`${c} month${c!==1?"s":""} ago`:`${d} year${d!==1?"s":""} ago`}}customElements.define(et,g);const ot="fn-head",L=document.createElement("template");L.innerHTML=`
  <style>
    :host {
      display: flex;
...
`;class st extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}).appendChild(L.content.cloneNode(!0))}}customElements.define(ot,st);const nt="fn-icon",$=document.createElement("template");$.innerHTML=`
  <style>
    :host {
      display: inline-flex;
      align-items: center;
  ...
  <span></span>
`;class it extends HTMLElement{constructor(){super();i(this,"icons",{external:`
    ...
    `},r=new Blob([s[n]],{type:"image/svg+xml"}),a=URL.createObjectURL(r),h=new Blob([s[`${n}Click`]],{type:"image/svg+xml"}),l=URL.createObjectURL(h),c=new Blob([s.pointer],{type:"image/svg+xml"}),d=URL.createObjectURL(c),H=new Blob([s.pointerClick],{type:"image/svg+xml"}),M=URL.createObjectURL(H),R=new Blob([s.text],{type:"image/svg+xml"}),S=URL.createObjectURL(R);o.style.setProperty("--cursor-default",`url(${a}), auto`),o.style.setProperty("--cursor-default-click",`url(${l}), auto`),o.style.setProperty("--cursor-pointer",`url(${d}), auto`),o.style.setProperty("--cursor-pointer-click",`url(${M}), auto`),o.style.setProperty("--cursor-text",`url(${S}), auto`)}function at(){rt()}document.addEventListener("DOMContentLoaded",at);const dt={parameters:{controls:{matchers:{color:/(background|color)$/i,date:/Date$/i}},docs:{theme:A.dark}}};export{dt as default};
