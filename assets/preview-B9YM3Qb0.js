...
`;class I extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}).appendChild(p.content.cloneNode(!0))}}customElements.define(_,I);const D="fn-action",v=document.createElement("template");v.innerHTML=`
  <style>
    ::slotted(a),
    ::slotted(button) {
  ...
  <slot></slot>
`;class m extends HTMLElement{constructor(){super();r(this,"elem",null);r(this,"pressing",!1);r(this,"shortcut",null);r(this,"disabled",!1);r(this,"href");this.attachShadow({mode:"open"}).appendChild(v.content.cloneNode(!0))}static get observedAttributes(){return["disabled","decoration","button","fill","focus"]}connectedCallback(){var t;this.elem=this.querySelector("a, button"),this.shortcut=(t=this.elem.querySelector("em"))==null?void 0:t.textContent,this._handleMouseDown=this.handleMouseDown.bind(this),this._handleKeyDown=this.handleKeyDown.bind(this),this._handleKeyUp=this.handleKeyUp.bind(this),this._handleShortcutKeyDown=this.handleShortcutKeyDown.bind(this),this._handleShortcutKeyUp=this.handleShortcutKeyUp.bind(this),this.elem.addEventListener("mousedown",this._handleMouseDown),this.elem.addEventListener("keydown",this._handleKeyDown),this.elem.addEventListener("keyup",this._handleKeyUp),this.shortcut&&(document.addEventListener("keydown",this._handleShortcutKeyDown),document.addEventListener("keyup",this._handleShortcutKeyUp)),m.observedAttributes.forEach(e=>{this.hasAttribute(e)&&this.attributeChangedCallback(e,null,this.getAttribute(e))})}disconnectedCallback(){this.elem.removeEventListener("mousedown",this._handleMouseDown),this.elem.removeEventListener("keydown",this._handleKeyDown),this.elem.removeEventListener("keyup",this._handleKeyUp),this.shortcut&&(document.removeEventListener("keydown",this._handleShortcutKeyDown),document.removeEventListener("keyup",this._handleShortcutKeyUp))}attributeChangedCallback(t,e,o){if(this.elem)switch(t){case"disabled":this.handleDisable(o==="true");break;case"decoration":this.style.setProperty("--decoration",o);break;case"button":o==="true"?this.elem.classList.add("button"):this.elem.classList.remove("button");break;case"focus":o==="true"&&this.elem.focus();break;case"fill":o==="true"?this.elem.classList.add("fill"):this.elem.classList.remove("fill");break}}handleShortcutKeyDown(t){if(!this.disabled&&!this.pressing&&t.key.toLowerCase()===this.shortcut.toLowerCase()){const e=this.elem.getAttribute("href");this.pressing=!0,this.elem.classList.add("pressed"),e&&this.handlePrefetch(e)}}handleShortcutKeyUp(t){this.disabled||this.pressing&&t.key.toLowerCase()===this.shortcut.toLowerCase()&&(this.elem.classList.remove("pressed"),this.elem.click(),this.pressing=!1)}handleMouseDown(){if(this.disabled)return;const t=this.elem.getAttribute("href");t&&this.handlePrefetch(t)}handleKeyDown(t){if(this.disabled||this.pressing)return;const e=this.elem.getAttribute("href");switch(t.key){case"Enter":t.preventDefault(),this.elem.classList.add("pressed"),e&&this.handlePrefetch(e);break}this.pressing=!0}handleKeyUp(t){this.disabled||(t.key==="Enter"&&(t.preventDefault(),this.elem.classList.remove("pressed"),this.elem.click()),this.pressing=!1)}handlePrefetch(t){if(this.dataset.prefetched||!t||t.startsWith("#")||/^(http|https):\/\/[^ "]+$/.test(t)&&new URL(t).origin!==window.origin)return;const e=Object.assign(document.createElement("link"),{rel:"prefetch",href:t});document.head.append(e),this.dataset.prefetched=!0}handleDisable(t=!0){if(this.disabled=t,t===!0)this.elem.tagName==="A"?(this.elem.setAttribute("aria-disabled",!0),this.elem.removeAttribute("href")):this.elem.setAttribute("disabled",!0);else if(this.elem.tagName==="A"){const e=this.elem.getAttribute("href");this.elem.removeAttribute("aria-disabled"),this.elem.setAttribute("href",e)}else this.elem.removeAttribute("disabled")}}customElements.define(D,m);const P="fn-header",f=document.createElement("template");f.innerHTML=`
  <style>
    :host {
      align-items: center;
      display: flex;
...
`;class B extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}).appendChild(f.content.cloneNode(!0))}}customElements.define(P,B);const V="fn-image",w=document.createElement("template");w.innerHTML=`
  <style>
    :host {
      --corner: 1rem;
...
`;class K extends HTMLElement{constructor(){super();r(this,"loaded",!1);r(this,"observer",new IntersectionObserver(t=>{t.forEach(e=>{e.isIntersecting&&(this.shadowRoot.querySelector(".wrapper").classList.add("loaded"),this.handleGlow(),this.loaded=!0,this.observer.disconnect())})}));this.attachShadow({mode:"open"}).appendChild(w.content.cloneNode(!0))}async connectedCallback(){var e,o;if(this.loaded)return;this.image=this.querySelector("img"),this._handlePoster=this.handlePoster.bind(this),this._handleLoad=()=>{this.observer.observe(this.image)};const t=this.getAttribute("poster");t&&(this.image.style.visibility="hidden",this.poster=new Image,this.poster.src=t,this.poster.complete?this._handlePoster():this.poster.addEventListener("load",this._handlePoster)),(e=this.image)!=null&&e.complete?this._handleLoad():(o=this.image)==null||o.addEventListener("load",this._handleLoad)}disconnectedCallback(){var t,e;(t=this.image)==null||t.removeEventListener("load",this._handleLoad),(e=this.poster)==null||e.removeEventListener("load",this._handlePoster),this.observer.disconnect()}handlePoster(){const t=this.shadowRoot.querySelector("canvas#poster"),e=t.getContext("2d"),{width:o,height:n}=this.image;t.width=o,t.height=n;const a=.05,h=t.width*a,l=t.height*a;e.drawImage(this.poster,0,0,h,l),e.msImageSmoothingEnabled=!1,e.mozImageSmoothingEnabled=!1,e.webkitImageSmoothingEnabled=!1,e.imageSmoothingEnabled=!1,e.drawImage(t,0,0,h,l,0,0,t.width,t.height);const c=getComputedStyle(this).getPropertyValue("--color-fg");e.globalCompositeOperation="color",e.fillStyle=c,e.fillRect(0,0,t.width,t.height),t.classList.add("loaded"),this.image.style.visibility="visible"}handleGlow(){const t=this.shadowRoot.querySelector("canvas#glow"),e=t.getContext("2d"),{width:o,height:n}=this.image;t.width=o,t.height=n,t.style.filter="blur(1.5rem) opacity(0.5)",e.drawImage(this.image,0,0,o,n),t.classList.add("loaded")}}customElements.define(V,K);const q="fn-columns",y=document.createElement("template");y.innerHTML=`
  ...
  <slot name="row"></slot>
`;class j extends HTMLElement{static get observedAttributes(){return["columns"]}constructor(){super(),this.attachShadow({mode:"open"}).appendChild(y.content.cloneNode(!0))}connectedCallback(){this.style.setProperty("--size",this.children[0].children.length),[...this.children].forEach(s=>{s.style.setProperty("display","grid"),s.style.setProperty("grid-gap","var(--gap)"),s.style.setProperty("grid-template-columns","repeat(var(--columns), 1fr)")})}}customElements.define(q,j);const O="fn-hero",b=document.createElement("template");b.innerHTML=`
  <style>
    :host {
      display: block;
      position: relative;
...
`;class Z extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}).appendChild(b.content.cloneNode(!0))}connectedCallback(){this.image=this.querySelector("img"),this.image.style.width="100%",this.image.style.height="auto"}}customElements.define(O,Z);const z="fn-section",x=document.createElement("template");x.innerHTML=`
  ...
  <slot></slot>
`;class X extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}).appendChild(x.content.cloneNode(!0))}}customElements.define(z,X);const Y="fn-timeago",k=document.createElement("template");k.innerHTML=`
  <style>
    :host {
      font: var(--font-accent);
      text-transform: uppercase;
...
`;class u extends HTMLElement{constructor(){super();r(this,"timeElement",null);this.attachShadow({mode:"open"}).appendChild(k.content.cloneNode(!0)),this.timeElement=this.shadowRoot.querySelector("time")}static get observedAttributes(){return["date"]}attributeChangedCallback(t,e,o){if(t==="date"){const n=parseInt(o,10),a=new Date(n);this.timeElement.setAttribute("datetime",a.toISOString()),this.timeElement.textContent=u.timeAgo(o)}}static timeAgo(t){const o=Date.now()-t,n=Math.floor(o/1e3),a=Math.floor(n/60),h=Math.floor(a/60),l=Math.floor(h/24),d=Math.floor(l/30),c=Math.floor(l/365);return n<60?"just now":a<60?`${a} minute${a!==1?"s":""} ago`:h<24?`${h} hour${h!==1?"s":""} ago`:l<30?`${l} day${l!==1?"s":""} ago`:d<12?`${d} month${d!==1?"s":""} ago`:`${c} year${c!==1?"s":""} ago`}}customElements.define(Y,u);const F="fn-head",C=document.createElement("template");C.innerHTML=`
  <style>
    :host {
      display: flex;
...
`;class W extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}).appendChild(C.content.cloneNode(!0))}}customElements.define(F,W);const G="fn-icon",L=document.createElement("template");L.innerHTML=`
  <style>
    :host {
      display: inline-flex;
      align-items: center;
  ...
  <span></span>
`;class J extends HTMLElement{constructor(){super();r(this,"icons",{external:`
...
`).length*50}get state(){return this._state}set state(t){this._state=t,this.setAttribute("data-state",t),this.dispatchEvent(new CustomEvent(t,{bubbles:!0}))}attributeChangedCallback(t,e,o){if(t==="disabled"){if(e===o)return;o==="true"?(this.stopTimer(),this.observer.disconnect()):this.observer.observe(this.root)}}startTimer(){this.timer=setTimeout(()=>{this.setAsViewed()},this.time)}stopTimer(){clearTimeout(this.timer)}setAsViewed(){this.observer.disconnect(),this.state="viewed"}}customElements.define(Q,tt);function et(i,s){const t={default:`
    ...
    `,text:`
     <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 12.14 32.67" width="10.14" height="30.67">
      <polygon fill="${s}" points=".15 32.54 .15 30.88 1.55 30.88 1.55 29.48 5.26 29.48 5.26 26.57 2.93 26.57 2.93 25.38 5.26 25.38 5.26 3.2 2.91 3.2 2.91 3.19 1.53 3.19 1.53 1.78 .12 1.78 .12 .13 1.78 .13 1.78 1.53 3.19 1.53 3.19 2 8.94 2 8.94 1.53 10.34 1.53 10.34 .13 12 .13 12 1.79 10.6 1.79 10.6 3.19 9.22 3.19 9.1 3.2 6.92 3.2 6.92 25.38 9.24 25.38 9.24 26.57 6.92 26.57 6.92 29.48 10.62 29.48 10.62 30.88 12.02 30.88 12.02 32.54 10.36 32.54 10.36 31.14 8.96 31.14 8.96 30.68 3.21 30.68 3.21 31.14 1.81 31.14 1.81 32.54 .15 32.54"/>
      <path fill="${i}" d="M1.66.25v1.4h1.41v.47h6v-.47h1.4V.25h1.41v1.41h-1.4v1.4h-1.38s-2.3,0-2.3,0v22.43h2.33v.95h-2.33v3.16h3.7v1.4h1.4v1.41h-1.41v-1.4h-1.4v-.46H3.09v.46h-1.41v1.4H.27v-1.41h1.41v-1.4h3.7v-3.16h-2.33v-.95h2.33V3.07h-2.35s-1.38,0-1.38,0v-1.4H.25V.25h1.41M1.91,0H0v1.91h1.41v1.4h1.63s2.1,0,2.1,0v21.93h-2.33v1.45h2.33v2.66H1.43v1.4H.02v1.91h1.91v-1.4h1.41v-.46h5.5v.46h1.4v1.4h1.91v-1.91h-1.4v-1.4h-3.7v-2.66h2.33v-1.45h-2.33V3.32h2.3s1.13,0,1.13,0h.25v-1.4h1.4V0h-1.91v1.4h-1.4v.47H3.32v-.47h-1.41V0h0Z"/>
    </svg>
    `},e=new Blob([t.default],{type:"image/svg+xml"}),o=URL.createObjectURL(e),n=new Blob([t.defaultClick],{type:"image/svg+xml"}),a=URL.createObjectURL(n),h=new Blob([t.pointer],{type:"image/svg+xml"}),l=URL.createObjectURL(h),d=new Blob([t.pointerClick],{type:"image/svg+xml"}),c=URL.createObjectURL(d),$=new Blob([t.text],{type:"image/svg+xml"}),M=URL.createObjectURL($);document.documentElement.style.setProperty("--cursor-default",`url(${o}), auto`),document.documentElement.style.setProperty("--cursor-default-click",`url(${a}), auto`),document.documentElement.style.setProperty("--cursor-pointer",`url(${l}), auto`),document.documentElement.style.setProperty("--cursor-pointer-click",`url(${c}), auto`),document.documentElement.style.setProperty("--cursor-text",`url(${M}), auto`)}function st(i,s){var o;const t=i,e=s;document.documentElement.style.setProperty("--color-fg",t),document.documentElement.style.setProperty("--color-bg",e),(o=document.querySelector('meta[name="theme-color"]'))==null||o.setAttribute("content",e),et(t,e)}const nt={parameters:{controls:{matchers:{color:/(background|color)$/i,date:/Date$/i}},docs:{theme:S.dark}}};st("#ffb000","#111111)");export{nt as default};
